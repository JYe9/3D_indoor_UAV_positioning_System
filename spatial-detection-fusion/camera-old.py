...
class Camera:
    ...
    def __init__(self, device_info: dai.DeviceInfo, friendly_id: int, show_video: bool = True):
        ...
        print("=== Connected to Camera[{}] - mxid: {}".format(self.friendly_id, self.mxid))
        

    def __del__(self):
        self.device.close()
        print("=== Closed Camera[{}] - mxid: {}".format(self.friendly_id, self.mxid))
    ...
    def _create_pipeline(self):
        ...
        blob = Path(__file__).parent.joinpath("models", "best_openvino_2022.1_6shave.blob")#"newdrone416_openvino_2021.4_6shave.blob"
        ...
        spatial_nn.setConfidenceThreshold(0.6) #confidence threshold setting
        ...
        edgeDetectorRgb.setMaxOutputFrameSize(cam_rgb.getVideoWidth() * cam_rgb.getVideoHeight())
    ...
    def update(self):
        ...
        for detection in detections:    
            ...
            edgeFrame, target_x, target_y = find_largest_contour(edgeFrame, bbox)
            

            if target_x != -999 and target_y != -999:
                x_angle_offset = (target_x - (self.NN_IMG_SIZE / 2.0)) * 68.7938003540039 / 640 #angle_offset = (target_x - (self.NN_IMG_SIZE / 2.0)) * 68.7938003540039 / 1920
                y_angle_offset = -(target_y - (self.NN_IMG_SIZE / 2.0)) * 68.7938003540039 / 640
                ...
                # if abs(x_angle_offset) > 30:
                #     print("Invalid angle offset. Setting it to 0")
                #     # x_angle_offset = 0
                # else:
                #     # print("Found target '{}'\tX Angle Offset: {}".format(target_label, angle_offset))
                #     print("Detected'{}': X_Angle:{:.3f} Y_Angle:{:.3f} x:{}mm y:{}mm z:{}mm".format("Drone", x_angle_offset, y_angle_offset, x_distance, y_distance, z_distance))   
                
                # print("Detected'{}': X_Angle:{:.3f} Y_Angle:{:.3f} x:{}mm y:{}mm z:{}mm".format("Drone", x_angle_offset, y_angle_offset, x_distance, y_distance, z_distance))
                print("[{}] Camera: \u2220Horizontal:{:.3f}\N{DEGREE SIGN} \u2220Vertical:{:.3f}\N{DEGREE SIGN} x:{}mm y:{}mm z:{}mm".format(self.friendly_id, x_angle_offset, y_angle_offset, x_distance, y_distance, z_distance))
                # Get a list of DepthAI Oak devices
                ...
                # output = " ".join([f"camera{self.friendly_id}:{z_distance}mm"])
                output = f"{z_distance}"
                ...
                #         # while z_distance != 0:
                #         #     t = conn.sendall(f"{z_distance}".encode())
                #         #     time.sleep(1)
                #         #     conn.close()
            ...
            # cv2.imshow("edgeFrame",edgeFrame)
            ...
            cv2.putText(visualization, str(label), (x1 + 10, y1 + 20), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 0, 255))
            cv2.putText(visualization, "conf:{:.2f}%".format(detection.confidence*100), (x1 + 10, y1 + 35), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 0, 0))
            cv2.putText(visualization, f"X: {int(detection.spatialCoordinates.x)} mm", (x1 + 10, y1 + 50), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 0, 0))
            cv2.putText(visualization, f"Y: {int(detection.spatialCoordinates.y)} mm", (x1 + 10, y1 + 65), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 0, 0))
            cv2.putText(visualization, f"Z: {int(detection.spatialCoordinates.z)} mm", (x1 + 10, y1 + 80), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 0, 0))
            cv2.putText(visualization, "x_angle:{:.3f}degree".format(x_angle_offset), (x1 + 10, y1 + 95), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 0, 0))
            cv2.putText(visualization, "y_angle:{:.3f}degree".format(y_angle_offset), (x1 + 10, y1 + 110), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 0, 0))
